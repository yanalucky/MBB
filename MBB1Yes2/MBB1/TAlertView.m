...
+ (void)initialize
{
    ...
    appearance.titleFont            = [UIFont systemFontOfSize:15];
...
-(void)addSubviewsWithOverlay:(BOOL)withOverlay
{
    ...
    switch (_style) {
        ...
        case TAlertViewStyleNeutral:
             titleColor = [UIColor colorWithRed:109/255.0 green:181/255.0 blue:195/255.0 alpha:1];
        default:
            break;
    }
    ...
    CGRect aletViewFrame = CGRectMake(self.frame.size.width/2-AlerViewWidth/2, outsideMargin * 4,AlerViewWidth, 0);
    ...
    UIImageView *iv = [[UIImageView alloc]initWithFrame:CGRectMake(_alertView.frame.size.width/2-57/2, yPos, 57, 52)];
    iv.image = [UIImage imageNamed:_imageName ofSex:_sex];
    [_alertView addSubview:iv];
    yPos +=60;
    
    
    if (_title) {
        _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, yPos,AlerViewWidth, 0)];
