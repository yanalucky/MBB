...
//    self.backgroundColor = MBColor(251, 239, 240, 1);
...
- (void)commonInit
{
    ...
    {
        CYCircleView = [JTCircleView new];
        CYCircleView.color = MBColor(251, 239, 240, 1);
        [self addSubview:CYCircleView];
...
- (void)configureConstraintsForSubviews{
    ...
    CYCircleView.frame = CGRectMake(0, 0, sizeCircle, sizeCircle);
    CYCircleView.center = CGPointMake(self.frame.size.width / 2., self.frame.size.height / 2.);
    CYCircleView.layer.cornerRadius = sizeCircle / 2.;
    CYCircleView.hidden = self.calendarManager.calendarAppearance.isHiddenCYView;
    
    dotView.frame = CGRectMake(0, 0, sizeDot, sizeDot);
    dotView.center = CGPointMake(self.frame.size.width / 2., (self.frame.size.height / 2.) + sizeDot * 2.5 + 4);
...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated
{
    ...
    if(selected){
        ...
        if(!self.isOtherMonth){
            ...
            circleView.color = MBColor(251, 239, 240, 1);
    ...
    if(animated){
        ...
        circleView.layer.opacity = opacity;
        circleView.layer.shadowColor = [UIColor magentaColor].CGColor;
