...
{
    static Fcgo_AddressPickerView *_sharedClient = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedClient = [[Fcgo_AddressPickerView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, KScreenHeight) selectAddressBlock:nil];
    });
...
{
    Fcgo_AddressPickerView *picker = [Fcgo_AddressPickerView sharedClient];
...
- (instancetype)initWithFrame:(CGRect)frame selectAddressBlock:(void(^)(NSMutableDictionary *addressDict))addressBlock
{
    ...
    if (self)
    {
        ...
        [self addTarget:self action:@selector(hideMyPicker) forControlEvents:UIControlEventTouchUpInside];
