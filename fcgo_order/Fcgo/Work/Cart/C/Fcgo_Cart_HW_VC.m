...
@implementation Fcgo_Cart_HW_VC
...
-(void)setDirectDict:(NSDictionary *)directDict
{
    _directDict = directDict;
    //结束刷新
    [self.kjAndHWView.tableview.mj_header endRefreshing];
    [self.noneview.collectionview.mj_header endRefreshing];
    if (!directDict) {
    ...
    NSArray *normalArray = directDict[@"normal"];
    NSArray *intetalArray = directDict[@"intetal"];
...
- (void)viewDidLoad
{
    ...
    if (self.directDict) {
        [self setDirectDict:self.directDict];
    }
}
...
- (void)prepareWithModel:(Fcgo_CartModel *)model
{
    ...
    NSDictionary *dict = @{@"goodsNumber":[NSString stringWithFormat:@"%ld",count],
