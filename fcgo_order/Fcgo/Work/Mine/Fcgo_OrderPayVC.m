...
@interface Fcgo_OrderPayVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation Fcgo_OrderPayVC
...
- (void)viewDidLoad {
    ...
    [self requestData:^{
        if (!weakSelf.model) {
            ...
            [WSProgressHUD showImage:nil status:@"获取数据失败，请重试!"];
        }
        else {
...
- (void)acceptPay {
    ...
    vc.orderId = [NSString stringWithFormat:@"%@",self.model.orderId];
...
- (void)pay {
    ...
    [paremtes  setObjectWithNullValidate:self.model.orderId forKey:@"order"];
    [paremtes  setObjectWithNullValidate:payType forKey:@"payType"];
//    [paremtes setObjectWithNullValidate:self.orderNo forKey:@"orderNo"];
...
- (void)requestData:(void(^)(void))block {
    ...
    [param setObjectWithNullValidate:self.orderNo forKey:@"orderNo"];
    [WSProgressHUD showWithStatus:@"加载中..." maskType:(WSProgressHUDMaskTypeDefault)];
    [Fcgo_NetworkManager postRequest:NSFormatHeardMeThodUrl(ServiceLocalTypeOne, ORDERMETHOD, @"pay") parametersContentCommon:param successBlock:^(BOOL success, id responseObject, NSString *msg) {
        if (success) {
            NSDictionary *dict = responseObject[@"data"];
            if (dict.count) {
                self.model = [Fcgo_OrderPayModel yy_modelWithDictionary:dict];
            }
        }
        !block?:block();
    } failureBlock:^(NSString *description) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        Fcgo_OrderPaySection0Row1Cell *cell = [tableView dequeueReusableCellWithIdentifier:@"orderPaySection0Row1Cell"];
        if (self.model) {
            cell.model = self.model;
            cell.timeFinishBlock = ^{
                [WSProgressHUD showImage:nil status:@"订单支付已逾期！"];
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    Fcgo_OrderPaySection2Row1Cell *deselectCell = [tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:self.selectIndex inSection:1]];
...
- (void)setupUI {
    ...
    [self.navigationView setupBackBtnBlock:^{
        [Fcgo_AlertAnimationView showWithTitle:@"确认放弃支付吗" text:@"超过订单支付时效后，订单将被取消，请尽快完成支付" cancelTitle:@"继续支付" confirmTitle:@"放弃" block:^{
            [weakSelf.navigationController popViewControllerAnimated:YES];
        }];
    }];
    [self.navigationView setupTitleLabelWithTitle:@"支付中心"];
    
    [self.table registerNib:[UINib nibWithNibName:NSStringFromClass([Fcgo_OrderPaySection0Row1Cell class]) bundle:nil] forCellReuseIdentifier:@"orderPaySection0Row1Cell"];
