...
@interface Fcgo_footPrintVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation Fcgo_footPrintVC
...
- (void)loadGoodsListRequest
{
    ...
    [Fcgo_NetworkManager postRequest:NSFormatHeardMeThodUrl(ServiceLocalTypeOne,FOOTPRINTMETHOD, @"list") parametersContentCommon:paremtes successBlock:^(BOOL success, id responseObject, NSString *msg) {
...
{
    [self showNoControl:!isShow titleString:@"暂无足迹" imageString:@"ico_no-collect"];
    self.table.hidden = !isShow;
}
...
//        if (weakSelf.goodsListMutableArray.count<=0) {
//            [WSProgressHUD showImage:nil status:@"暂无足迹"];
//            return ;
//        }
//        [Fcgo_AlertAnimationView showWithTitle:@"提示" text:@"这是清空所有足迹列表的操作呦" cancelTitle:@"取消" confirmTitle:@"确定" block:^{
...
//    }];
    ...
    [self.navigationView setupRightBtnTitle:@"清空" Block:^{
        ...
        [Fcgo_AlertAnimationView showWithTitle:@"提示" text:@"清空所有足迹列表?" cancelTitle:@"取消" confirmTitle:@"确定" block:^{
            [weakSelf deleteCollectGoodsWithGoodsIds:@"" block:^{}];
            [self.goodsListMutableArray removeAllObjects];
            [self.table reloadData];
        }];
    ...
    self.navigationView.navRightTitleBtn.hidden = 1;
    [self.navigationView setupTitleLabelWithTitle:@"我的足迹"];
...
- (void)deleteCollectGoodsWithGoodsIds:(NSString *)goodsIds block:(void(^)(void))block
{
    ...
    [Fcgo_NetworkManager postRequest:NSFormatHeardMeThodUrl(ServiceLocalTypeOne,FOOTPRINTMETHOD, @"del") parametersContentCommon:paremtes successBlock:^(BOOL success, id responseObject, NSString *msg)  {
        if (success) {
