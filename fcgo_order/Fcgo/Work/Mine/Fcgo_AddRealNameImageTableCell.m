//
//  Fcgo_AddRealNameImageTableCell.m
//  Fcgo
//
...
@interface Fcgo_AddRealNameImageTableCell ()<HXPhotoViewControllerDelegate,ALiImageReshapeDelegate>
...
@implementation Fcgo_AddRealNameImageTableCell
...
- (void)setupUI
{
    ...
    self.adviceLabel.textColor = UIFontMiddleGrayColor;
    
    [self.mainCardBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_offset(15);
        make.top.mas_equalTo(weakSelf.adviceLabel.mas_bottom).mas_offset(15);
    ...
    [self.desLabel1 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_offset(15);
        make.top.mas_equalTo(weakSelf.mainCardImageView.mas_bottom).mas_offset(15);
        make.right.mas_offset(-15);
    }];
    self.desLabel1.textColor = UIFontMiddleGrayColor;
    ...
    self.desLabel2.textColor = UIFontMiddleGrayColor;
}
...
- (void)setModel:(Fcgo_RealNameModel *)model
{
    _model = model;
    WEAKSELF(weakSelf)
    [self.mainCardBtn sd_setBackgroundImageWithURL:[NSURL URLWithString:self.model.mchPicurlW] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"add_ID_realName"]completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
         weakSelf.mainImage = image;
    }];
    
    [self.backCardBtn sd_setBackgroundImageWithURL:[NSURL URLWithString:self.model.mchPicurlB] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"add_ID_realName"]completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
         weakSelf.backImage = image;
    }];
}
