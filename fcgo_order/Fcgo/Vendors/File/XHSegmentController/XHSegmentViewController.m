...
- (XHSegmentControl *)segmentControl
{
    ...
    {
//
        _segmentControl = [[XHSegmentControl alloc] initWithFrame:CGRectMake(0, (self.haveNavBar)?0:(kNavigationHeight - 44), [UIScreen mainScreen].bounds.size.width, DefaultSegmentHeight)];
...
- (void)setIsNav:(BOOL)isNav
{
    ...
    if (isNav)
    {
       self.segmentControl.frame = CGRectMake(0, kNavigationHeight, [UIScreen mainScreen].bounds.size.width, DefaultSegmentHeight);
    }
    else
    {
        //        kNavigationHeight - 44
        self.segmentControl.frame = CGRectMake(0, (self.haveNavBar)?0:(kNavigationHeight - 44), [UIScreen mainScreen].bounds.size.width, DefaultSegmentHeight);
    }
}
...
//    self.scrollView.contentSize = CGSizeMake(CGRectGetWidth(self.scrollView.frame) * self.viewControllers.count, CGRectGetHeight(view.frame) + 100);
