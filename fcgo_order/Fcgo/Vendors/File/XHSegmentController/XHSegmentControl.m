...
- (void)initialize
{
    ...
    UILabel  *lineLab = [[UILabel alloc]initWithFrame:CGRectMake(0, 43.5, kScreenWidth,0.5)];
...
- (void)load
{
    ...
    //  load 高亮线
    self.lineLayer.backgroundColor = self.highlightColor.CGColor;
     self.lineLayer.frame = CGRectMake(CGRectGetMinX(self.lineLayer.frame), CGRectGetHeight(self.frame) - self.lineWidth, 40, self.lineWidth);
...
- (void)setSelectIndex:(NSInteger)selectIndex animation:(BOOL)animation
{
    ...
    if (selectIndex < self.items.count) {
        ...
        self.lineLayer.frame = CGRectMake(item.center.x-20, CGRectGetHeight(item.frame) - self.lineWidth, 40, self.lineWidth);
...
- (void)setCurrentSelectIndex:(NSInteger)currentSelectIndex
{
    ...
    [self setSelectIndex:currentSelectIndex animation:NO];
}
