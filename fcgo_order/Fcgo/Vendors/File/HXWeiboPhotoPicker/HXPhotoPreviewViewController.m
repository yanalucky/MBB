...
- (void)setup
{
    ...
    if (self.manager.selectedList.count > 0) {
        ...
        [self.rightBtn setTitle:[NSString stringWithFormat:@"下一步(%ld)",(unsigned long)self.manager.selectedList.count] forState:UIControlStateNormal];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    self.titleLb.text = [NSString stringWithFormat:@"%d/%ld",currentIndex + 1,(unsigned long)self.modelList.count];
...
- (UILabel *)titleLb
{
    if (!_titleLb) {
        ...
        _titleLb.text = [NSString stringWithFormat:@"%d/%ld",self.index + 1,(unsigned long)self.modelList.count];
    }
    return _titleLb;
}
...
- (void)didSelectedClick:(UIButton *)button
{
    ...
    if (!button.selected) {
        if (self.manager.selectedList.count == self.manager.maxNum) {
            // 已经达到最大选择数
            [self.view showImageHUDText:[NSString stringWithFormat:@"最多只能选择%ld个",(long)self.manager.maxNum]];
            return;
        }
        if (self.manager.type == HXPhotoManagerSelectedTypePhotoAndVideo) {
            if ((model.type == HXPhotoModelMediaTypePhoto || model.type == HXPhotoModelMediaTypePhotoGif) || (model.type == HXPhotoModelMediaTypeCameraPhoto || model.type == HXPhotoModelMediaTypeLivePhoto)) {
                ...
                if (self.manager.selectedPhotos.count == self.manager.photoMaxNum) {
                    [self.view showImageHUDText:[NSString stringWithFormat:@"最多只能选择%ld张图片",(long)self.manager.photoMaxNum]];
                    // 已经达到图片最大选择数
                    return;
                }
            }
